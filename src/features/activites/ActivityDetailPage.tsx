...
function formatContent(text: string) {
  return text.split('\n\n').map((paragraph, i) => {
    if (paragraph.startsWith('—') || paragraph.includes('\n—')) {
      ...
      return (
        <ul key={i} className="space-y-2 my-4">
          {items.map((item, j) => (
            <li key={j} className="flex items-start gap-2 text-[#2D1F14] font-sans">
    ...
    if (paragraph.startsWith('«') || paragraph.includes('libère')) {
      return (
        <blockquote key={i} className="border-l-4 border-[#C8912A] pl-5 my-6 italic font-serif text-lg text-[#5C3D2E]">
          {paragraph}
        </blockquote>
      )
    }
    return <p key={i} className="text-[#2D1F14] font-sans leading-relaxed mb-4">{paragraph}</p>
  })
}
          ...
          <Container>
            <div style={{ opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(20px)', transition: 'opacity 0.7s ease, transform 0.7s ease' }}>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                <Badge variant="ocre">{activity.type}</Badge>
              ...
              <div className="sticky top-24 space-y-4">
                <div className="bg-white rounded-sm border border-[#D4C4A8] p-6">
                  ...
                  <div className="space-y-4">
                    {[
                      { icon: <Clock size={16} className="text-[#C8912A]" />, label: 'Durée', value: activity.duration },
                      { icon: <Users size={16} className="text-[#C8912A]" />, label: 'Participants', value: activity.participants },
                      { icon: <Tag size={16} className="text-[#C8912A]" />, label: 'Tarif', value: activity.price },
                      { icon: <MapPin size={16} className="text-[#C8912A]" />, label: 'Lieu', value: activity.location },
                    ].map(info => (
        ...
        <div className="bg-[#F5EDD8] py-16">
          <Container>
            <h2 className="font-serif text-2xl text-[#5C3D2E] mb-8">
              D&apos;autres propositions de {activity.owner.name.split('(')[0].trim()}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {related.map(rel => (
                <Link key={rel.slug} href={`/activites/${rel.slug}`} className="group block">
