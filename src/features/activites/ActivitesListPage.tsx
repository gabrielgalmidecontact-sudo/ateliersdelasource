...
// Static data with PDF content — replaced by CMS in production
const allActivities = [
  {
    code: 'A1',
    ...
    owner: 'Gabriel',
    excerpt: 'Avec un groupe de 4 à 5 personnes, Gabriel vous accompagne dans une immersion profonde au cœur de vous-même. Un processus collectif, conscient et créatif pour désamorcer les mécanismes répétitifs.',
  ...
  },
  {
    ...
    owner: 'Gabriel',
    excerpt: 'Formé pendant 3 ans à la biographie avec Cyr Boé, Gabriel vous propose des entretiens d\'environ une heure pour explorer le sens de votre vie et découvrir les rythmes qui la traversent.',
    duration: '1 heure',
  ...
  },
  {
    ...
    owner: 'Gabriel',
    excerpt: 'Gagner en aisance corporelle et verbale pour un oral, une audition, un entretien… ou simplement pour retrouver une manière d\'être plus libre et plus tranquille au quotidien.',
    duration: '1 heure',
    participants: 'Individuel',
    type: 'Atelier',
  ...
  },
  {
    ...
    owner: 'Gabriel (Galmide)',
    excerpt: 'Un seul en scène semi-improvisé où Galmide raconte 7 années de vie rocambolesque — déjantée, délurée, touchante et absolument vraie. Le public participe. Au chapeau.',
    duration: '1h30',
  ...
  },
  {
    ...
    owner: 'Gabriel (Galmide)',
    excerpt: 'Immersion poétique à travers l\'œuvre magistrale de Victor Hugo, portée par Galmide et accompagnée d\'une violoncelliste ou pianiste. 1h30. Jouable dans vos salons. Au chapeau.',
    duration: '1h30',
    participants: 'Tous publics',
    type: 'Spectacle',
  ...
  },
  {
    ...
    slug: 'massages-soins',
    owner: 'Amélie',
    excerpt: 'Amélie vous accueille dans son espace de soins corporels. Détails et horaires à venir dès l\'ouverture de sa salle.',
    duration: 'À définir',
    participants: 'Individuel',
    type: 'Soin',
  ...
  },
  {
    ...
    slug: 'hebergement',
    owner: 'Amélie',
    excerpt: 'Informations pour réserver une nuit ou un séjour sur le lieu. Contenu à venir prochainement.',
    duration: 'Variable',
  ...
  },
  {
    ...
    owner: 'Amélie',
    excerpt: 'Accès, itinéraire et informations pratiques pour rejoindre les Ateliers de la Source. Contenu à venir.',
    duration: '—',
    participants: '—',
    type: 'Informations',
...
function ActivityCard({ activity, index }: { activity: typeof allActivities[0]; index: number }) {
  return (
    <motion.div
    ...
    >
      <Link href={`/activites/${activity.slug}`} className="group block h-full" aria-label={`Voir l'activité : ${activity.title}`}>
        <article className="h-full flex flex-col bg-white rounded-sm border border-[#D4C4A8] hover:border-[#C8912A]/50 hover:shadow-lg transition-all duration-300 overflow-hidden">
          ...
          <div className="relative h-52 overflow-hidden bg-[#D4C4A8]">
            ...
            <div className="absolute top-3 left-3">
              <Badge variant={typeColors[activity.type] || 'default'}>{activity.type}</Badge>
            </div>
            {/* Code badge */}
      ...
      <Section bg="creme" id="gabriel">
        <Container>
          <motion.div
            ...
            className="mb-10"
          >
            <p className="text-xs font-sans tracking-widest uppercase text-[#C8912A] mb-2">Avec Gabriel</p>
            <h2 className="font-serif text-2xl md:text-3xl text-[#5C3D2E]">Stages, Ateliers &amp; Spectacles</h2>
            <div className="mt-3 w-12 h-0.5 bg-gradient-to-r from-[#D4AF50] to-transparent" />
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {gabriel.map((act, i) => <ActivityCard key={act.code} activity={act} index={i} />)}
          </div>
        </Container>
      </Section>
      ...
      <Section bg="beige" id="amelie">
        <Container>
          <motion.div
            ...
            className="mb-10"
          >
            <p className="text-xs font-sans tracking-widest uppercase text-[#C8912A] mb-2">Avec Amélie</p>
            <h2 className="font-serif text-2xl md:text-3xl text-[#5C3D2E]">Soins, Hébergement &amp; Accueil</h2>
            <div className="mt-3 w-12 h-0.5 bg-gradient-to-r from-[#D4AF50] to-transparent" />
            <p className="mt-3 text-sm font-sans text-[#7A6355] max-w-lg">
              Les propositions d&apos;Amélie seront prochainement détaillées. N&apos;hésitez pas à la contacter directement.
            </p>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {amelie.map((act, i) => <ActivityCard key={act.code} activity={act} index={i} />)}
          </div>
        </Container>
      </Section>
    </>
