...
const defaultGabriel: FounderPerson = {
  name: 'Gabriel',
...
const defaultAmelie: FounderPerson = {
  name: 'Amélie',
...
function PersonBlock({ name, role, bio, imageUrl, activities, isPlaceholder }: FounderPerson) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        ...
        {activities.map((a) => <ActivityLink key={`${a.code}-${a.slug}`} {...a} />)}
      </div>
      <div className="mt-6">
...
type FounderColumnsProps = {
  gabriel?: FounderPerson
  amelie?: FounderPerson
}

export function FounderColumns({ gabriel, amelie }: FounderColumnsProps) {
  const { ref, visible } = useFadeIn()
  const gabrielData = gabriel ?? defaultGabriel
  const amelieData = amelie ?? defaultAmelie
