// src/app/api/member/questionnaires/route.ts
// Member : liste des questionnaires actifs et ses soumissions
import { NextRequest, NextResponse } from 'next/server'
import { createServerClient } from '@/lib/supabase/server'

async function getUser(req: NextRequest) {
  const token = req.headers.get('authorization')?.replace('Bearer ', '')
  if (!token) return null
  const supabase = createServerClient()
  const { data: { user } } = await supabase.auth.getUser(token)
  if (!user) return null
  return { user, supabase }
}

// GET — questionnaires actifs + soumissions du membre
export async function GET(req: NextRequest) {
  const ctx = await getUser(req)
  if (!ctx) return NextResponse.json({ error: 'Non authentifié' }, { status: 401 })

  const [tplRes, subRes] = await Promise.all([
    ctx.supabase
      .from('questionnaire_templates')
      .select('*, questionnaire_questions(*)')
      .eq('is_active', true)
      .order('created_at', { ascending: false }),
    ctx.supabase
      .from('questionnaire_submissions')
      .select('*')
      .eq('member_id', ctx.user.id),
  ])

  if (tplRes.error) {
    if (tplRes.error.code === '42P01') return NextResponse.json({ questionnaires: [], submissions: [] })
    return NextResponse.json({ error: tplRes.error.message }, { status: 400 })
  }

  return NextResponse.json({
    questionnaires: tplRes.data || [],
    submissions: subRes.data || [],
  })
}
